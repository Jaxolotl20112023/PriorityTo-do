...
class button : 
    ...
    def __init__(self, screen, width, height, x, y, content):
        self.screen = screen
        self.width = width
        self.height = height
        self.x = x 
...
class textField: 
    ...
    def change(self, tasks): 
        ...
        for lines in range(0,len(tasks)) : 
            self.d_buttons.append(button(self.screen,50,25,self.x+wrapLength,self.y,"done"))
...
b_addTask = button(screen, 100, 50, SCREEN_WIDTH/2, SCREEN_HEIGHT-100,"Add Task")
b_addTask.draw()
